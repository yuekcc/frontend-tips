...
其中 `PageHeader` 是自定义组件。`el-button` 是 element-ui 的一个按钮组件。
...
commit、push 是多人协作开发的第一步。就算需求很大，也是可以拆分多个阶段，每个阶段至少可以进行
一次 commit/push。代码尽快集成也是减少冲突的方法。分次 commit 也更方便使用 git 的 cherry-pick 等
特性，补丁更方便。
